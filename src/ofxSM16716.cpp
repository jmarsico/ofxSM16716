...
bool ofxSM16716::init(){
  ...
  // setup Arduino with data pins for each strip
  for (int k = 0; k < NUM_STRIPS; k++) {
    pinMode(DATA_PIN + k, OUTPUT);
    digitalWrite(DATA_PIN + k, LOW);
  }
...
void ofxSM16716::set_pixel(uint8_t strip, uint8_t index, uint32_t color) {
  pixels[strip][index] = color & 0x00FFFFFF;
}
...
void ofxSM16716::set_pixel_rgb(uint8_t strip, uint8_t index, uint32_t r, uint32_t g, uint32_t b) {
  
  if(r > 255) r = 255;
  if(g > 255) g = 255;
  if(b > 255) b = 255;

  set_pixel(strip, index, b | (g << 8) | (r << 16));
}
...
void ofxSM16716::write_pixel(uint8_t strip, uint8_t i) {
  ...
  uint32_t p = pixels[strip][i] | MASK;
  ...
  while (j--) {
    ...
    digitalWrite(DATA_PIN + strip, (p & MASK) ? HIGH : LOW);
...
void ofxSM16716::write_blank_pixel() {
  ...
  while (j--) {
    for (int k = 0; k < NUM_STRIPS; k++) {
      // write a single 0 to each strip in turn
      digitalWrite(DATA_PIN + k, 0);
    }
    toggle_clock();
  }
}

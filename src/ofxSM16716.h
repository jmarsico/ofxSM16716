...
class ofxSM16716 {
  public:
    ...
    void set_pixel(uint8_t strip, uint8_t index, uint32_t color);

    bool init();
    
    // set a pixel in the RBG array using separate uints for R, G, and B; does not write to grid
    void set_pixel_rgb(uint8_t strip, uint8_t index, uint32_t r, uint32_t g, uint32_t b);
    ...
    void write_pixel(uint8_t strip, uint8_t i);
  ...
  private:
    ...
    // this represents the 
    uint32_t pixels[NUM_STRIPS][LIGHT_COUNT];
 };
